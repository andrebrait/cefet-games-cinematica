...
public class Cinematica extends ApplicationAdapter {
	...
	public void create() {
		...
		fugir = new Fugir(40, 2, 200);
		fugir.alvo = buscar.alvo;
		chegar = new Chegar(40, 16, 0.5f);
		...
		stringAlgoritmoCorrente = "Algoritmo corrente: " + algoritmoCorrente.getNome();
		...
		Gdx.input.setInputProcessor(new InputAdapter() {
			...
			public boolean touchDown(int x, int y, int pointer, int button) {
				...
				if (button == Buttons.RIGHT) {
					...
					while (it.hasNext() && !definiuObjetivo) {
						atual = it.next();
						definiuObjetivo = Colisoes
								.colideCom(new Circle(new Vector2(atual.pose.posicao.x, atual.pose.posicao.y),
										RenderizadorAgente.RAIO), clique);
					}
					if (definiuObjetivo) {
			...
			public boolean scrolled(int quanto) {
				...
				camera.update();
				viewport.setWorldSize(camera.zoom * camera.viewportWidth, camera.zoom * camera.viewportHeight);
				return true;
			}
			...
			public boolean keyUp(int keycode) {
				...
				for (AlgoritmoMovimentacao algoritmo : algoritmos) {
					if (keycode == algoritmo.getTeclaParaAtivacao()) {
						...
						stringAlgoritmoCorrente = "Algoritmo corrente: " + algoritmo.getNome();
						return true;
					}
				}
				// teclas gerais
	...
	/**
	 * Faz com que o mundo seja infinito. Quando um agente chega ao final do mundo
	 * (1 dos 4 cantos), ele é teleportado para o outro lado.
	 *
	 * @param agente
	 *            Agente
	 */
